class stage2 extends Phaser.Scene {
	...
	create() {
		...
		// player object
		this.player = this.add.image(config.width / 2, config.height * 3 / 4, "player");
		...
		this.ghost = this.add.image(config.width / 2, config.height / 2, "ghost");
		this.physics.add.existing(this.ghost, false);

		this.ghostAngle = 0;
		this.xmultiplier = 1
		this.ymultiplier = 1
		// this.ghostAngleY = 0;
		// this.ghost.playAnimation("ghost_walk", [0, 1]);
		...
		this.time.addEvent({ delay: 250, callback: this.ghostShoot, callbackScope: this, loop: true });
	...
	ghostShoot() {
		...
		var dummySpeed = 1000;
		// var bulletSpeed = 10;
		var candy = this.add.image(config.width / 2, config.height / 2, "candy");
		this.physics.add.existing(candy, false);
		
		
		candy.body.setMaxSpeed(150);
		

		if ( (dummySpeed - (this.ghostAngle*100)) == 1000 ){
			this.ghostAngle = 0;
		}
		if ( (dummySpeed - (this.ghostAngle*100)) == -1000 ){
			this.xmultiplier = -1
		...
		if ( (dummySpeed - (this.ghostAngle*100)) == 0 ){
			this.ymultiplier = -1
		}
		candy.body.setVelocityX(dummySpeed + (this.xmultiplier *(this.ghostAngle*100)));
		candy.body.setVelocityY(0 + (this.ymultiplier *(this.ghostAngle*100)));
		// candy.body.setVelocityY(0 + (this.ghostAngleX*100));
		// candy.body.setVelocityY(50);

		// candy.body.setVelocityX((MAX_SPEED - (this.ghostAngle*25)));
		// candy.body.setVelocityY((0 + (this.ghostAngle*25)));

		// console.log(IMAGINARY_SPEED - (this.ghostAngleX*100));
		// if ((dummySpeed - (this.ghostAngleX*100)) < -100) {
		// 	this.ghostAngleY = 0;
		// }
		// if ((dummySpeed - (this.ghostAngleY*100)) < 100) {
		// 	this.ghostAngleX = 0;
		// }
